...
% ---- sanity checks:
if( size( Event_i ) ~= 4 || size( Event_f ) ~= 4 || )
...
S=unique( state_vector_ti_si( Event_i(1)+1:Event_f(1) ) )
...
if( state_vector_ti_si( Event_i(1)+1 ) -  state_vector_ti_si( Event_i(1) ) <=0  || state_vector_ti_si( Event_f(1)+1 ) -  state_vector_ti_si( Event_i(1) ) >=0 )
