...
Nevents      = size( SoE, 1 );
Nsubtracks   = size( SoE_statemat_in, 2 );
...
while ( ~check_states_tenable (SoE_statemat_out)   )
    ...
    [ alpha_tracks ] = SoE_get_alpha_merge_list( SoE, broken_track, breaking_event);
    ...
    SoE_statemat_out = SoE_time_evolve_statemat ( SoE, Nsubtracks, adj_mat );
